...
- **Bank removal:** Borrower can request removal of a **verified** account (reason required) only if **another verified account** exists; creates **`BankRemovalRequest`** for **`/admin/bank_removal_requests`**. Approve sets bank status **`removed`** (excluded from new draws); reject leaves it verified. **`/bank_removal_requests`** lists the company’s requests.
